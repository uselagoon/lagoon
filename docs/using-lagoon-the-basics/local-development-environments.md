...
DDEV is an open source, advanced `cli` and `tui` for Docker-based PHP development environments. It has a large feature set and an extensive set of add-ons. Recommended for those that prefer ease of use and a large community over production similarity.
