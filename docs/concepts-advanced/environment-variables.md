...
Scopes are used to set an env var for a specific context. An env var can have a scope of `buildtime`, `runtime`, `global`, `container_registry` or `internal_container_registry`.
...
An example of setting a `buildtime` env var:

=== "Lagoon CLI"

    ```bash title=""
    lagoon add variable --project lagoon-demo --scope buildtime \
...
An example of setting a build var:
