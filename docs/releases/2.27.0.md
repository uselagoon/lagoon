...
The `remote-controller` can get into a state where all deployments are put in the queue and never started. The solution is to roll the image back to `uselagoon/remote-controller:v0.24.0`.
