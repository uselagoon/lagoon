...
pipeline {
  ...
  stages {
    ...
    stage ('deploy to test environment') {
      ...
      steps {
        sh script: 'output=$(curl -s -X POST -F token=$TOKEN -F ref=main https://git.amazeeio.cloud/api/v4/projects/345/trigger/pipeline); echo "$output"; [[ "$output" == *"project_id"* ]] || exit 1', label: "Trigger lagoon-core helmfile sync on amazeeio-test6"
      }
    }
