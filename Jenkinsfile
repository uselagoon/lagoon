      ...
      parallel {
        stage ('0: setup test cluster') {
          steps {
            ...
            sh script: "make k3d/stable-local-stack INSTALL_SEED_DATA=false BRANCH_NAME=${SAFEBRANCH_NAME} LAGOON_CORE_USE_HTTPS=false INSTALL_PROMETHEUS=false INSTALL_AERGIA=false", label: "Setup stable cluster"
            sh script: "make k3d/retest TESTS=[nginx] BRANCH_NAME=${SAFEBRANCH_NAME} USE_STABLE_TESTS=false", label: "Run nginx smoketest"
            sh script: "make k3d/install-lagoon BRANCH_NAME=${SAFEBRANCH_NAME} INSTALL_PROMETHEUS=false INSTALL_AERGIA=false", label: "Run lagoon upgrade"
    ...
    stage ('run second test suite') {
      ...
      parallel {
        stage ('2: run second test suite') {
          steps {
            sh script: "make -j$NPROC k3d/retest TESTS=[bulk-deployment,image-cache,services,services-2,ssh-legacy,tasks,project-clone] BRANCH_NAME=${SAFEBRANCH_NAME}", label: "Running second test suite on k3d cluster"
