...
function initSendToAmazeeioTasks() {
	...
	var channelWrapper = connection.createChannel({
		setup: function setup(channel) {
			...
			channel.assertQueue('amazeeio-tasks:deploy-openshift', { durable: true }), channel.bindQueue('amazeeio-tasks:deploy-openshift', 'amazeeio-tasks', 'deploy-openshift'),

			// Queue for messages with `remove-openshift-resources` routing key
			channel.assertQueue('amazeeio-tasks:remove-openshift-resources', { durable: true }), channel.bindQueue('amazeeio-tasks:remove-openshift-resources', 'amazeeio-tasks', 'remove-openshift-resources'),

			// wait queues for handling retries
			channel.assertExchange('amazeeio-tasks-retry', 'direct', { durable: true }), channel.assertQueue('amazeeio-tasks:retry-queue', { durable: true, arguments: { 'x-dead-letter-exchange': 'amazeeio-tasks' } }), channel.bindQueue('amazeeio-tasks:retry-queue', 'amazeeio-tasks-retry', 'deploy-openshift'), channel.bindQueue('amazeeio-tasks:retry-queue', 'amazeeio-tasks-retry', 'remove-openshift-resources')]);
		}
	});
...
function createDeployTask(deployData) {
	var siteGroupName, branchName, sha, type, activeSystems, activeSystem, activeDeploySystem, deploySystemConfig, branchRegex;
		...
		while (1) {
			switch (_context2.prev = _context2.next) {
				...
				case 6:
					...
					_context2.next = _context2.t0 === 'lagoon_openshift' ? 11 : 31;
					break;

				case 11:
					deploySystemConfig = activeSystems.deploy['lagoon_openshift'];
					...
					_context2.t1 = deploySystemConfig.branches;
				...
				case 22:
					...
					branchRegex = new RegExp(deploySystemConfig.branches);
		...
		while (1) {
			switch (_context3.prev = _context3.next) {
				...
				case 6:
					...
					_context3.next = _context3.t0 === 'lagoon_openshift' ? 11 : 12;
					break;
			...
			switch (_context5.prev = _context5.next) {
				case 0:
					onMessage = function _callee2(msg) {
						var failCount, retryExpirationSecs, retryExpirationMilisecs, retryMsgOptions;
							...
							while (1) {
								switch (_context4.prev = _context4.next) {
									...
									case 6:
										...
										failCount = msg.properties.headers["x-death"] && msg.properties.headers["x-death"][0]['count'] ? msg.properties.headers["x-death"][0]['count'] + 1 : 1;
									...
									case 13:
										retryExpirationSecs = Math.pow(10, failCount);
										retryExpirationMilisecs = retryExpirationSecs * 1000;


										try {
											retryHandler(msg, _context4.t0, failCount, retryExpirationSecs);
										} catch (error) {}
