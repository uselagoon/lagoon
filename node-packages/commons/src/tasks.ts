...
export const getControllerBuildData = async function(deployTarget: any, deployData: DeployData) {
  ...
  // check if the project has an orgkey
  const projectOrgKey = await getOrganizationKeyByProjectName(projectName);
  const orgKey: any = R.prop('getOrganizationKeyByProjectName', projectOrgKey)
  if (orgKey?.privateKey) {
    // if so, ship it in the build payload
    buildDeployData.spec.project.organizationKey = encodeBase64(orgKey.privateKey.replace(/\\n/g, "\n"))
  }
