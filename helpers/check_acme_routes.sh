...
CLUSTER_API_HOSTNAME="${CLUSTER_API_HOSTNAME:-""}"
...
function usage() {
	echo -e "The available commands are:
		...
		By default, script runs routes' checks on ch.amazee.io towards API endpoint ch.amazeeio.cloud.
...
# Function to check if you are running the script on the right cluster
...
#	set -x
	...
	do
		...
		if echo $ROUTE_HOSTNAME_IP | egrep -q -v "${CLUSTER_IPS[*]}"; then
...
function notify_customer() {
	...
	MESSAGE="Your $ROUTE_HOSTNAME route is configured in the \`.lagoon.yml\` file to issue an TLS certificate from Let's Encrypt. Unfortunately Lagoon is unable to issue a certificate as $DNS_ERROR. \n
...
function main() {
	...
	echo "You're running the script on $CLUSTER_HOSTNAME"
	check_cluster_api
