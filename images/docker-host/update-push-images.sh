...
docker login -u=serviceaccount --password-stdin ${REGISTRY} < /var/run/secrets/kubernetes.io/serviceaccount/token
...
for FULL_IMAGE in $(docker image ls --format "{{.Repository}}:{{.Tag}}" | grep -E "${REPOSITORY_TO_UPDATE}/" | grep -v none); do
  ...
  docker tag ${FULL_IMAGE} ${REGISTRY}/${NAMESPACE}/${IMAGE_NO_REPOSITORY}
  # Push to the openshift registry
  docker push ${REGISTRY}/${NAMESPACE}/${IMAGE_NO_REPOSITORY}
done
