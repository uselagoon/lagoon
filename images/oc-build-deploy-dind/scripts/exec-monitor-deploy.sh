...
stream_logs_deployment() {
  ...
  do
    ...
    oc -n ${OPENSHIFT_PROJECT} get --insecure-skip-tls-verify pods -l deployment=${SERVICE_NAME}-${LATEST_VERSION} -o json | jq -r '.items[] | .metadata.name + " " + .spec.containers[].name' | while read -r POD CONTAINER ; do
        oc -n ${OPENSHIFT_PROJECT} logs --insecure-skip-tls-verify --timestamps -f $POD -c $CONTAINER 2> /dev/null >> /tmp/oc-build-deploy/logs/container/${SERVICE_NAME}/$POD-$CONTAINER.log &
    done
...
if [[ $ret -ne 0 ]]; then
  ...
  find /tmp/oc-build-deploy/logs/container/${SERVICE_NAME}/ -type f -print0 | xargs -0 -I % sh -c 'echo ======== % =========; cat %; echo'
  exit 1
fi
