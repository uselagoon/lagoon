...
stream_logs_daemonset() {
  ...
  do
    ...
    oc -n ${OPENSHIFT_PROJECT} get --insecure-skip-tls-verify pods -l "pod-template-generation=${GENERATION},service=${DAEMONSET}" -o json | jq -r '.items[] | .metadata.name + " " + .spec.containers[].name' | while read -r POD CONTAINER ; do
...
while true; do
    if [[ $SECONDS -gt $END ]]; then
      ...
      find /tmp/oc-build-deploy/logs/container/${DAEMONSET}/ -type f -print0 | xargs -0 -I % sh -c 'echo ======== % =========; cat %; echo'
      exit 1
    fi
