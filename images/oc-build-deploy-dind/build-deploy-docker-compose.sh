...
if oc --insecure-skip-tls-verify -n ${OPENSHIFT_PROJECT} get schedules.backup.appuio.ch &> /dev/null; then
  ...
  if [ ! -z $MONTHLY_BACKUP_RETENTION ] && [ ! $LAGOON_ENVIRONMENT_TYPE = 'development']; then
  ...
  if [ ! -z $WEEKLY_BACKUP_RETENTION ] && [ ! $LAGOON_ENVIRONMENT_TYPE = 'development']; then
  ...
  if [ ! -z $DAILY_BACKUP_RETENTION ] && [ ! $LAGOON_ENVIRONMENT_TYPE = 'development']; then
