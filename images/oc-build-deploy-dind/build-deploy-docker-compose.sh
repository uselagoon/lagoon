...
PRIVATE_DOCKER_REGISTRIES=($(cat .lagoon.yml | shyaml keys private-registries || echo ""))
for PRIVATE_DOCKER_REGISTRY in "${PRIVATE_DOCKER_REGISTRIES[@]}"
do
  # check if a url is set, if none set proceed against docker hub
  PRIVATE_DOCKER_REGISTRY_URL=$(cat .lagoon.yml | shyaml get-value private-registries.$PRIVATE_DOCKER_REGISTRY.url false)
  if [ $PRIVATE_DOCKER_REGISTRY_URL == "false" ]; then
    echo "No 'url' defined for registry $PRIVATE_DOCKER_REGISTRY, will proceed against docker hub";
  fi
  # check the username and passwords are defined in yaml
  PRIVATE_DOCKER_REGISTRY_USERNAME=$(cat .lagoon.yml | shyaml get-value private-registries.$PRIVATE_DOCKER_REGISTRY.username false)
  if [ $PRIVATE_DOCKER_REGISTRY_USERNAME == "false" ]; then
    echo "No 'username' defined for registry $PRIVATE_DOCKER_REGISTRY"; exit 1;
  fi
  PRIVATE_DOCKER_REGISTRY_PASSWORD=$(cat .lagoon.yml | shyaml get-value private-registries.$PRIVATE_DOCKER_REGISTRY.password false)
  if [[ $PRIVATE_DOCKER_REGISTRY_PASSWORD == "false" ]]; then
    echo "No 'password' defined for registry $PRIVATE_DOCKER_REGISTRY"; exit 1;
  fi
  ...
  if [ $PRIVATE_DOCKER_REGISTRY_PASSWORD != "false" ]; then
    ...
    if [ ! -z "$LAGOON_PROJECT_VARIABLES" ]; then
      PRIVATE_REGISTRY_CREDENTIAL=($(echo $LAGOON_PROJECT_VARIABLES | jq -r '.[] | select(.scope == "docker" and .name == "'$PRIVATE_DOCKER_REGISTRY_PASSWORD'") | "\(.value)"'))
    fi
    if [ ! -z "$LAGOON_ENVIRONMENT_VARIABLES" ]; then
      PRIVATE_REGISTRY_CREDENTIAL=($(echo $LAGOON_ENVIRONMENT_VARIABLES | jq -r '.[] | select(.scope == "docker" and .name == "'$PRIVATE_DOCKER_REGISTRY_PASSWORD'") | "\(.value)"'))
    fi
    if [ -z $PRIVATE_REGISTRY_CREDENTIAL ]; then
      #if no password defined in the lagoon api, pass the one in `.lagoon.yml` as a password
      PRIVATE_REGISTRY_CREDENTIAL=$PRIVATE_DOCKER_REGISTRY_PASSWORD
    fi
    if [ $PRIVATE_DOCKER_REGISTRY_URL != "false" ]; then
      echo "Attempting to log in to $PRIVATE_DOCKER_REGISTRY_URL with user $PRIVATE_DOCKER_REGISTRY_USERNAME - $PRIVATE_DOCKER_REGISTRY_PASSWORD"
      docker login --username $PRIVATE_DOCKER_REGISTRY_USERNAME --password $PRIVATE_REGISTRY_CREDENTIAL $PRIVATE_DOCKER_REGISTRY_URL
    else
      echo "Attempting to log in to docker hub with user $PRIVATE_DOCKER_REGISTRY_USERNAME - $PRIVATE_DOCKER_REGISTRY_PASSWORD"
      docker login --username $PRIVATE_DOCKER_REGISTRY_USERNAME --password $PRIVATE_REGISTRY_CREDENTIAL
    fi
  fi
done
set +x
