...
get_dockerhost() {
  ...
  if timeout 1 ping -c1 docker.for.mac.localhost &> /dev/null; then
  ...
  if timeout 1 ping -c1 docker.for.win.localhost &> /dev/null; then
  ...
  if timeout 1 ping -c1 172.16.172.16 &> /dev/null; then
    echo "172.16.172.16"
    return
  fi
