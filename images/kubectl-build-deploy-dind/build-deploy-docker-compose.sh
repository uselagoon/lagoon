...
# so just add https...
ALL_ROUTES=$(build-deploy-tool identify ingress)
ROUTE=$(echo "${ALL_ROUTES}" | jq -r '.primary' )
ROUTES=$(echo "${ALL_ROUTES}" | jq -r '.secondary | join(",")' )
AUTOGENERATED_ROUTES=$(echo "${ALL_ROUTES}" | jq -r '.autogenerated | join(",")' )
