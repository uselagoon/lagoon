...
# so just add https...
ROUTE=$(build-deploy-tool identify ingress | jq -r '.primary' )
ROUTES=$(build-deploy-tool identify ingress | jq -r '.secondary | join(",")' )
AUTOGENERATED_ROUTES=$(build-deploy-tool identify ingress | jq -r '.autogenerated | join(",")' )
