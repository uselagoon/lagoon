...
exports.up = async function (knex) {
  ...
  if (!DBExists) {
    return knex.schema
      .createTable('advanced_task_definition', (table) => {
      ...
      .createTable('advanced_task_definition_argument', (table) => {
        table.increments('id').notNullable().primary();
      ...
      .createTable('backup_restore', (table) => {
        table.increments('id').notNullable().primary();
      ...
      .createTable('deploy_target_config', (table) => {
        table.increments('id').notNullable().primary();
        table.integer('project');
      ...
      .createTable('environment_backup', (table) => {
        table.increments('id').notNullable().primary();
        table.integer('environment');
      ...
      .createTable('environment_fact', (table) => {
        table.increments('id').notNullable().primary();
      ...
      .createTable('environment_fact_reference', (table) => {
        table.increments('id').notNullable().primary();
        table.integer('fid').notNullable();
        table.string('name', 300).notNullable();
        table.unique(['fid', 'name'], { indexName: 'fid' });
      })
      .createTable('environment_problem', (table) => {
        table.increments('id').notNullable().primary();
        table.integer('environment');
      ...
      .createTable('environment_service', (table) => {
        table.increments('id').notNullable().primary();
        table.integer('environment').notNullable();
        table.string('name', 100).notNullable();
      })
      .createTable('environment_storage', (table) => {
        table.increments('id').notNullable().primary();
        table.integer('environment');
      ...
      .createTable('notification_email', (table) => {
        table.increments('id').notNullable().primary();
        table.string('name', 50).unique({ indexName: 'name' });
        table.string('email_address', 300);
      })
      .createTable('notification_microsoftteams', (table) => {
        table.increments('id').notNullable().primary();
        table.string('name', 50).unique({ indexName: 'name' });
        table.string('webhook', 512);
      })
      .createTable('notification_rocketchat', (table) => {
        table.increments('id').notNullable().primary();
        table.string('name', 50).unique({ indexName: 'name' });
        table.string('webhook', 300);
        table.string('channel', 300);
      })
      .createTable('notification_slack', (table) => {
        table.increments('id').notNullable().primary();
        table.string('name', 50).unique({ indexName: 'name' });
        table.string('webhook', 300);
        table.string('channel', 300);
      })
      .createTable('notification_webhook', (table) => {
        table.increments('id').notNullable().primary();
        table.string('name', 50).unique({ indexName: 'name' });
        table.string('webhook', 2000);
      })
      .createTable('openshift', (table) => {
        table.increments('id').notNullable().primary();
        table.string('name', 50).unique({ indexName: 'name' });
      ...
      .createTable('problem_harbor_scan_matcher', (table) => {
        table.increments('id').notNullable().primary();
        table.string('name', 100).notNullable();
      ...
      .createTable('project_notification', (table) => {
        table.integer('nid');
        table.integer('pid');
      ...
      .createTable('task_file', (table) => {
        table.integer('tid');
        table.integer('fid');
        table.primary(['tid', 'fid']);
      })
      .createTable('user_ssh_key', (table) => {
        table.specificType('usid', 'CHAR(36)');
        table.integer('skid');
        table.primary(['usid', 'skid']);
      })
      .createTable('workflow', (table) => {
        table.increments('id').notNullable().primary();
