...
const enhanceMiddleware = (...middleware) => {
  ...
  if (process.env.NODE_ENV === 'development' && process.env.REMOTE_DEV_SERVER !== "false") {
    // 'remotedev' is a container managed by docker-compose
    const composeEnhancers = composeWithDevTools({ realtime: true, hostname: 'remotedev', port: 9090 });
...
export default (initialState: State, sagaArgs: RootSagaArgs): ApiStore => {
  ...
  const store = createStore(
    reducer,
    enhanceMiddleware(sagaMiddleware),
  );
