import {
  unless, is, isNil, isEmpty, partialRight, complement,
} from 'ramda';
...
export const toNumber = (input: string | number): number => unless(isNumber, partialRight(parseInt, [10]), input) as number;

export const notArray = complement(isArray);
export const isNotNil = complement(isNil);
export const isNotEmpty = complement(isEmpty);

export const asyncPipe = (...functions: any[]) => (input: any) => functions.reduce((chain, func) => chain.then(func), Promise.resolve(input));
...
export const encodeBase64 = (data: string): string => Buffer.from(data, 'utf8').toString('base64');

export const decodeBase64 = (data: string): string => Buffer.from(data, 'base64').toString('utf8');

export const encodeJSONBase64 = (data: any): string => encodeBase64(JSON.stringify(data));

export const decodeJSONBase64 = (data: string): any => JSON.parse(decodeBase64(data));
