...
export const initSendToLagoonActions = function () {
  ...
  connection.on('connect', ({ url }) => logger.verbose('lagoon-actions: Connected to %s', url, {
    action: 'connected',
    url,
  }),
  );
...
export const initSendToLagoonTasks = function () {
  ...
  connection.on('connect', ({ url }) => logger.verbose('lagoon-tasks: Connected to %s', url, {
    action: 'connected',
    url,
  }),
  );
...
export const makeSafe = (string: string): string => string.toLocaleLowerCase().replace(/[^0-9a-z-]/g, '-');
...
export const truncateEnvironmentName = (projectName: string, environmentName: string): string => {
  let envName = makeSafe(environmentName);
  const overlength = 58 - projectName.length;
  if (envName.length > overlength) {
    const hash = sha1(envName).substring(0, 4);
...
export const seedNamespace = (projectName: string, environmentName: string): string => {
  const envName = truncateEnvironmentName(projectName, environmentName);
...
export const createMiscTask = async function (taskData: any) {
  ...
  const updatedKey = `deploytarget:${key}`;
  ...
  switch (updatedKey) {
    case 'deploytarget:restic:backup:restore':
      ...
      if (
        ...
        const [bucketNameRaw, isSharedBucket] = await getBaasBucketName(result.environment.project, result.environment.openshift);
        let bucketName = bucketNameRaw;
        if (isSharedBucket) {
...
export const createRetentionPolicyTask = async function (policyData: any) {
  ...
  switch (`deploytarget:${policyData.key}`) {
    case 'deploytarget:harborpolicy:update':
      ...
      const payloadBytes = Buffer.from(JSON.stringify(policyData.data.event).replace(/\n/g, '\n')).toString('base64');
