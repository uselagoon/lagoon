...
export const Sql = {
  selectFactByDatabaseId: (fid: number) => knex('environment_fact as f')
  ...
  selectFactsByEnvironmentId: ({ environmentId, keyFacts, limit }) => {
    const q = knex('environment_fact as f')
      .distinct(standardFactReturn)
      .leftJoin('environment_fact_reference as r', 'r.fid', '=', 'f.id')
  ...
  }) => knex('environment_fact').insert({
    environment, name, value, source, description, type, category, keyFact, service,
  }).toString(),
  deleteFact: (environment, name) => knex('environment_fact')
    .where({
      environment,
      name,
    })
  ...
  selectFactReferenceByDatabaseId: (id: number) => knex('environment_fact_reference')
    .where({ id })
    .toString(),
  selectFactReferenceByNameAndEnvironmentId: (name: string, environmentId: number) => knex('environment_fact_reference as r')
    .select('r.*', 'f.environment')
    .leftJoin('environment_fact as f', 'f.id', '=', 'r.fid')
    .where('r.name', '=', name)
    .andWhere('f.environment', '=', environmentId)
    .toString(),
  selectFactReferencesByFactId: (fid: number) => knex('environment_fact_reference')
    .where('fid', '=', fid)
    .toString(),
  insertFactReference: ({ fid, name }) => knex('environment_fact_reference')
    .insert({ fid, name })
    .onConflict(['name', 'fid'])
    .merge()
    .toString(),
  deleteFactReferencesByFactId: (fid) => knex('environment_fact_reference')
    .where({ fid })
    .del()
    .toString(),
};
