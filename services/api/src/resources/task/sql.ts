...
export const Sql = {
  selectTask: (id: number) => knex('task')
    .where('task.id', '=', id)
    .toString(),
  selectTaskByNameAndEnvironment: (name: string, environmentId: number) => knex('task')
  ...
  insertTask: ({
    ...
    .toString(),
  deleteTask: (id: number) => knex('task')
    .where('id', id)
    .del()
    .toString(),
  updateTask: ({ id, patch }: { id: number; patch: { [key: string]: any } }) => knex('task')
    .where('id', id)
    .update(patch)
    .toString(),
  selectPermsForTask: (id: number) => knex('task')
    .select({
      pid: 'environment.project', adminOnlyView: 'task.admin_only_view', deployTokenInjection: 'task.deploy_token_injection', projectKeyInjection: 'task.project_key_injection',
    })
  ...
  insertAdvancedTaskDefinitionArgument: ({
    ...
    .toString(),
  updateAdvancedTaskDefinition: ({ id, patch }: { id: number; patch: { [key: string]: any } }) => knex('advanced_task_definition')
    .where('id', id)
    .update(patch)
    .toString(),
  selectAdvancedTaskDefinitionEnvironmentLinkById: (id: number) => knex('task_registration')
    .where('task_registration.id', '=', id)
    .toString(),
  selectTaskRegistrationById: (id: number) => knex('task_registration')
    .where('task_registration.id', '=', id)
    .toString(),
  selectTaskRegistrationsByEnvironmentId: (id: number) => knex('advanced_task_definition')
    ...
    .toString(),
  selectTaskRegistrationByEnvironmentIdAndAdvancedTaskId: (environmentId: number, task: number) => knex('task_registration')
    .where('task_registration.environment', '=', environmentId)
    .where('task_registration.advanced_task_definition', '=', task)
    .toString(),
  selectAdvancedTaskDefinition: (id: number) => knex('advanced_task_definition')
    ...
    .toString(),
  selectAdvancedTaskDefinitionArguments: (id: number) => knex('advanced_task_definition_argument')
    .where('advanced_task_definition_argument.advanced_task_definition', '=', id)
    .toString(),
  selectAdvancedTaskDefinitionArgumentById: (id: number) => knex('advanced_task_definition_argument')
    .where('advanced_task_definition_argument.id', '=', id)
    .toString(),
  deleteAdvancedTaskDefinitionArgumentByTaskDef: (advanced_task_definition: number) => knex('advanced_task_definition_argument')
    .where('advanced_task_definition_argument.advanced_task_definition', '=', advanced_task_definition)
    .del()
    .toString(),
  selectAdvancedTaskDefinitionByName: (name: string) => knex('advanced_task_definition')
    .select(knex.raw('*, advanced_task_definition.admin_only_view XOR 1 as "show_ui", advanced_task_definition.deploy_token_injection as "admin_task"')) // use admin_only_view as show_ui for backwards compatability
  ...
  selectAdvancedTaskDefinitions: () => knex('advanced_task_definition')
    .select(knex.raw('*, advanced_task_definition.admin_only_view XOR 1 as "show_ui", advanced_task_definition.deploy_token_injection as "admin_task"')) // use admin_only_view as show_ui for backwards compatability
    .toString(),
  selectAdvancedTaskDefinitionsForEnvironment: (id: number) => knex('advanced_task_definition')
    .select(knex.raw('*, advanced_task_definition.admin_only_view XOR 1 as "show_ui", advanced_task_definition.deploy_token_injection as "admin_task"')) // use admin_only_view as show_ui for backwards compatability
    .where('environment', '=', id)
    .toString(),
  selectAdvancedTaskDefinitionsForProject: (id: number) => knex('advanced_task_definition')
    .select(knex.raw('*, advanced_task_definition.admin_only_view XOR 1 as "show_ui", advanced_task_definition.deploy_token_injection as "admin_task"')) // use admin_only_view as show_ui for backwards compatability
    .where('project', '=', id)
    .toString(),
  selectAdvancedTaskDefinitionsForGroups: (groups) => knex('advanced_task_definition')
    .select(knex.raw('*, advanced_task_definition.admin_only_view XOR 1 as "show_ui", advanced_task_definition.deploy_token_injection as "admin_task"')) // use admin_only_view as show_ui for backwards compatability
    .where('group_name', 'in', groups)
    .toString(),
  selectAdvancedTaskDefinitionsForSystem: () => knex('advanced_task_definition')
    .select(knex.raw('*, advanced_task_definition.admin_only_view XOR 1 as "show_ui", advanced_task_definition.deploy_token_injection as "admin_task"')) // use admin_only_view as show_ui for backwards compatability
    ...
    .toString(),
  deleteAdvancedTaskDefinition: (id: number) => knex('advanced_task_definition')
    .where('id', id)
    .del()
    .toString(),
  ...
  selectTaskHistoryRetention: (environment: number, retain: number) => knex('task')
  ...
  selectTaskHistoryRetentionDays: (environment: number, retain: number) => knex('task')
    .select('id', 'name', 'remote_id')
    .where('environment', environment)
    .where('admin_only_view', 0)
  ...
  selectTaskHistoryRetentionMonths: (environment: number, retain: number) => knex('task')
    .select('id', 'name', 'remote_id')
    .where('environment', environment)
    .where('admin_only_view', 0)
  ...
  selectTaskHistoryForEnvironment: (environment: number) => knex('task')
    .where('environment', '=', environment)
    .toString(),
  // same as select, except it deletes all tasks for the environment outside of the requested retain value
  deleteTaskHistory: (environment: number, retain: number) => knex('task')
    .select('id', 'name', 'remote_id')
    .where('environment', environment)
    .where('admin_only_view', 0)
    .whereNotIn('id', function () {
    ...
    .toString(),
  // same as select, except it deletes all tasks for the environment outside of the requested retain value
  deleteTaskHistoryDays: (environment: number, retain: number) => knex('task')
    .select('id', 'name', 'remote_id')
    .where('environment', environment)
    .where('admin_only_view', 0)
    .where(knex.raw(`created <= NOW() - INTERVAL ${retain} DAY`))
    ...
    .toString(),
  // same as select, except it deletes all tasks for the environment outside of the requested retain value
  deleteTaskHistoryMonths: (environment: number, retain: number) => knex('task')
    .select('id', 'name', 'remote_id')
    .where('environment', environment)
    .where('admin_only_view', 0)
    .where(knex.raw(`created <= NOW() - INTERVAL ${retain} MONTH`))
    ...
    .toString(),
  // same as select, except it deletes all tasks for the environment outside of the requested retain value
  deleteTaskHistoryForEnvironment: (environment: number) => knex('task')
    .where('environment', '=', environment)
    .delete()
    .toString(),
};
