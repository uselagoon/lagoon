...
export const updateEnvironment: ResolverFn = async (
  ...
  if (input.patch.autogeneratedRoutes || input.patch.autogeneratedPathRoutes || input.patch.disableRequestVerification) {
    const projectData = await projectHelpers(sqlClientPool).getProjectById(curEnv.project)
  ...
  let pathRoutes
  if (input.patch.autogeneratedPathRoutes) {
    pathRoutes = JSON.stringify(input.patch.autogeneratedPathRoutes)
  } else if (input.patch.autogeneratedPathRoutes === null) {
    pathRoutes = null
  }
  ...
  await query(
    ...
    Sql.updateEnvironment({
      ...
      patch: {
        ...
        autogeneratedRoutes: input.patch.autogeneratedRoutes,
        autogeneratedPathRoutes: pathRoutes,
        disableRequestVerification: input.patch.disableRequestVerification,
      }
    })
  );
...
export const getAutogeneratedPathRoutes: ResolverFn = async (
  ...
  if (!input?.autogeneratedPathRoutes) {
    return [];
  }
  return JSON.parse(input.autogeneratedPathRoutes);
};