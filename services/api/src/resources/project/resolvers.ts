...
export const addProject = async (
  ...
  try {
    // don't purge the cache after performing this, purge will happen at the end
    await models.GroupModel.addUserToGroup(user, group, 'maintainer', false);
  } catch (err) {
    logger.error(
...
export const deleteProject: ResolverFn = async (
  ...
  try {
    ...
    // await models.GroupModel.removeProjectFromGroups(pid, projectGroups, false);
  ...
  try {
    ...
    // don't purge the cache after performing this, purge will happen at the end
    await models.GroupModel.deleteGroup(group.id, false);
    OpendistroSecurityOperations(
      sqlClientPool,
      models.GroupModel
