...
export const addProject = async (
  ...
  if (input.organization != null) {
    ...
    if (input.autogeneratedRoutes || input.autogeneratedRoutesPullrequests || input.autogeneratedRoutePrefixes || input.autogeneratedPathRoutes || input.disableRequestVerification) {
      if (await Helpers(sqlClientPool).checkApiRoutesFeature(input.organization) === false) {
  ...
  try {
    projectDefaultGroup = await models.GroupModel.addGroup({
  ...
  let defaultUser;
  try {
    defaultUser = await models.UserModel.addUser({
      email: `default-user@${project.name}`,
      username: `default-user@${project.name}`,
      comment: `autogenerated user for project ${project.name}`
    });
    await models.GroupModel.addUserToGroup(defaultUser, projectDefaultGroup, 'maintainer');
  } catch (err) {
    logger.error(`Could not create default user for project ${project.name}: ${err.message}`)
  }

  // Check if an existing user has the public key linked to them
  ...
  if (!userId) {
    // Add the key to the default user
    try {
      ...
      await query(
        sqlClientPool,
        sshKeySql.addSshKeyToUser({ sshKeyId: insertId, userId: defaultUser.id })
      );
    } catch (err) {
      logger.error(
    ...
    try {
      ...
      await models.GroupModel.addUserToGroup(user, projectDefaultGroup, 'maintainer');
    } catch (err) {
      logger.error(
        `Could not link existing user to default project group for ${project.name}: ${err.message}`
      );
    }
  }
  ...
  if (!userAlreadyHasAccess && keycloakGrant) {
    ...
    try {
      await models.GroupModel.addUserToGroup(user, projectDefaultGroup, 'owner');
    } catch (err) {
      logger.error(
...
export const updateProject: ResolverFn = async (
  ...
  // this is used to set the beta feature flag on a project from the organization
  // this feature will eventually be made generally available and the feature flag will be removed
  if (autogeneratedRoutes || autogeneratedRoutesPullrequests || autogeneratedRoutePrefixes || autogeneratedPathRoutes || disableRequestVerification) {
    const projectData = await Helpers(sqlClientPool).getProjectById(id)
    if (await Helpers(sqlClientPool).checkApiRoutesFeature(projectData.organization) === false) {
      throw Error(`This feature is currently unavailable`)
    }
  }
  ...
  await query(
    ...
    Sql.updateProject({
      ...
      patch: {
        ...
        autogeneratedRoutes,
        autogeneratedRoutesPullrequests,
        autogeneratedRoutePrefixes: joinedPrefixes,
        autogeneratedPathRoutes: pathRoutes,
        disableRequestVerification,
      }
    })
  );
