...
export const Helpers = (sqlClientPool: Pool) => {
  ...
  const getRetentionPoliciesByScopeWithTypeAndLink = async (type: string, scope: string, scopeId: number) => {
    let rows; let gr; let or; let pr; let
      orgRows = [];
  ...
  */
  const getProjectIdsForAssociatedPolicyID = async (type: string, policyId: number, removal: boolean) => {
    const policyOverrides = []; // store all the policy overrides that this function will generate
    const projects = []; // store all the collected project ids so that we can use it to select other projects not in this list later on
        ...
        if (opids[0].projectIds != null) {
          for (const pid of opids[0].projectIds.split(',')) {
            ...
            if (removal && d && d.id === policyId) {
              ...
              const policy = { pid, updatePolicy: true, rpid: d.id };
              if (targetIndex !== -1) {
    ...
    if (updateProjects[0].projectIds != null) {
      ...
      for (const pid of projects) {
        if (removal) {
          ...
          policyOverrides.push({ pid, removePolicy: true });
        } else {
          policyOverrides.push({ pid, updatePolicy: true, rpid: policyId });
        }
      }
    }
  ...
  */
  const getRetentionPolicyChangesRequired = async (scopeId: number, scope: string, type: string, policyId: number, removal: boolean) => {
    ...
    const policyOverrides = []; // projects with override policies
    switch (scope) {
      case 'global':
