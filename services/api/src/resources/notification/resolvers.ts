...
const checkOrgNotificationPermission = async (hasPermission, input, adminScopes) => {
  ...
  } else if (DISABLE_NON_ORGANIZATION_NOTIFICATION_ASSIGNMENT === 'false' || adminScopes.platformOwner) {
...
export const addNotificationWebhook: ResolverFn = async (root, { input }, {
  sqlClientPool, hasPermission, adminScopes, userActivityLogger,
}) => {
  await checkOrgNotificationPermission(hasPermission, input, adminScopes);
...
export const addNotificationToProject: ResolverFn = async (
  ...
  const input = [
    ...
    (argumentsToProcess, functionToApply) => functionToApply(argumentsToProcess),
    unformattedInput,
  );
  ...
  projectNotification.contentType = input.contentType || NOTIFICATION_CONTENT_TYPE;
  projectNotification.notificationSeverityThreshold = input.notificationSeverityThreshold || NOTIFICATION_SEVERITY_THRESHOLD;
...
const deleteNotificationGeneric = async (sqlClientPool, notificationTableName, typename, name) => {
  const res = await query(sqlClientPool, knex(notificationTableName).where('name', name).toString());
  const nsid = R.path([0, 'id'], res);
  if (!nsid) {
    throw Error(`Notification of name ${name} not found`);
  }
  await query(sqlClientPool, knex('project_notification').where('nid', nsid).andWhere('type', typename).delete()
    .toString());
...
export const getNotificationsByProjectId: ResolverFn = async (
  ...
  const resultArray = results.reduce((acc, rows) => {
    if (rows == null) {
      return acc;
    }
  ...
  return resultArray.map(e => ({ ...e, notificationSeverityThreshold: notificationIntToContentType((e as any).notificationSeverityThreshold) }),
  );
};
...
export const getNotificationsByOrganizationId: ResolverFn = async (
  ...
  const results = await Promise.all(
    types.map(type => organizationHelpers(sqlClientPool).getNotificationsByTypeForOrganizationId(oid, type),
    ),
  );
...
export const updateNotificationMicrosoftTeams: ResolverFn = async (
  ...
  const rows = await query(
    sqlClientPool,
    Sql.selectNotificationMicrosoftTeamsByName(newName || name),
  );
...
export const updateNotificationWebhook: ResolverFn = async (
  ...
  const rows = await query(
    sqlClientPool,
    Sql.selectNotificationWebhookByName(newName || name),
  );
...
export const updateNotificationEmail: ResolverFn = async (
  ...
  const rows = await query(
    sqlClientPool,
    Sql.selectNotificationEmailByName(newName || name),
  );
...
export const updateNotificationRocketChat: ResolverFn = async (
  ...
  const rows = await query(
    sqlClientPool,
    Sql.selectNotificationRocketChatByName(newName || name),
  );
...
export const updateNotificationSlack: ResolverFn = async (
  ...
  const rows = await query(
    sqlClientPool,
    Sql.selectNotificationSlackByName(newName || name),
  );
