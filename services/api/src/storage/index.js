...
export function listYamlFiles(repoPath: string): Promise<Array<string>> {
  return new Promise((resolve, reject) => {
    glob(`${repoPath}/**/*.yaml`, (err, matches) => {
      if (err) {
        reject(err);
        return;
      }
      const ret = matches.map(filePath => path.relative(repoPath, filePath));
      resolve(ret);
    });
  });
}
