...
const typeDefs = gql`
  ...
  type Route {
    ...
    insecure: String
    """
    annotations are used by the route for things like tls-acme, hsts configuration, etc.
    """
    ...
    """
    Read only domains are only for reading, read only domains will not be provided as domains to a build. Turning read only off will send them to a build to be created or updated.
    """
    source: RouteSource
    """
    Verfied is a placeholder status for if a route is verified or not.
    """
    verified: String
    """
    verification will store the json payload data of the verification, this could include dns information etc of the verified domain(s).
    """
    verification: JSON
    wildcard: Boolean
    wildcardApex: Boolean
  ...
  input AddRouteToEnvironmentInput {
    domain: String!
    project: String!
    environment: String!
    service: String!
  ...
  type Mutation {
    ...
    addOrUpdateRouteToEnvironment(input: AddRouteToEnvironmentInput!): Route
