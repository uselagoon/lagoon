...
projects_in_deployment_target=$(echo "$projects_with_environments" | jq -c 'map(select(.openshift.name=="'$1'")) | .[]')

echo "$projects_in_deployment_target" | while read project
do
  PROJECT_NAME=$(echo "$project" | jq -r '.name')
  OPENSHIFT_NAME=$(echo "$project" | jq -r '.openshift.name')
  ...
  if [[ $PROJECT_NAME =~ $PROJECT_REGEX ]]; then
    ...
    echo "$OPENSHIFT_NAME|$OPENSHIFT_URL: Handling project $PROJECT_NAME"
