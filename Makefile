...
publish-testlagoon-images:
	PLATFORMS=$(PUBLISH_PLATFORM_ARCH) DATABASE_VENDOR=$(DATABASE_VENDOR) DATABASE_DOCKERFILE=$(DATABASE_DOCKERFILE) IMAGE_REPO=docker.io/testlagoon TAG=$(BRANCH_NAME) LAGOON_VERSION=$(LAGOON_VERSION) LAGOON_SYNC_GIT_BRANCH=$(LAGOON_SYNC_GIT_BRANCH) docker buildx bake -f docker-bake.hcl $(DOCKER_BUILD_CACHE) --builder $(CI_BUILD_TAG) --push

# tag and push all images
CI_IMAGE_REPO ?= docker.io/uselagoon

.PHONY: publish-uselagoon-images
publish-uselagoon-images:
	PLATFORMS=$(PUBLISH_PLATFORM_ARCH) DATABASE_VENDOR=$(DATABASE_VENDOR) DATABASE_DOCKERFILE=$(DATABASE_DOCKERFILE) IMAGE_REPO=$(CI_IMAGE_REPO) TAG=$(LAGOON_VERSION) LAGOON_VERSION=$(LAGOON_VERSION) LAGOON_SYNC_GIT_BRANCH=$(LAGOON_SYNC_GIT_BRANCH) docker buildx bake -f docker-bake.hcl $(DOCKER_BUILD_CACHE) --builder $(CI_BUILD_TAG) --push
	PLATFORMS=$(PUBLISH_PLATFORM_ARCH) DATABASE_VENDOR=$(DATABASE_VENDOR) DATABASE_DOCKERFILE=$(DATABASE_DOCKERFILE) IMAGE_REPO=$(CI_IMAGE_REPO) TAG=latest LAGOON_VERSION=$(LAGOON_VERSION) LAGOON_SYNC_GIT_BRANCH=$(LAGOON_SYNC_GIT_BRANCH) docker buildx bake -f docker-bake.hcl $(DOCKER_BUILD_CACHE) --builder $(CI_BUILD_TAG) --push

.PHONY: publish-uselagoon-images-pr
publish-uselagoon-images-pr:
	PLATFORMS=$(PUBLISH_PLATFORM_ARCH) DATABASE_VENDOR=$(DATABASE_VENDOR) DATABASE_DOCKERFILE=$(DATABASE_DOCKERFILE) IMAGE_REPO=$(CI_IMAGE_REPO) TAG=$(LAGOON_VERSION) LAGOON_VERSION=$(LAGOON_VERSION) LAGOON_SYNC_GIT_BRANCH=$(LAGOON_SYNC_GIT_BRANCH) docker buildx bake -f docker-bake.hcl $(DOCKER_BUILD_CACHE) --builder $(CI_BUILD_TAG) --push
.PHONY: clean
clean:
	rm -rf build/*
