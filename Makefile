...
minishift: local-dev/minishift/minishift
	$(info starting minishift $(MINISHIFT_VERSION) with name $(MINISHIFT_PROFILE))
	./local-dev/minishift/minishift --profile $(MINISHIFT_PROFILE) start --cpus $(MINISHIFT_CPUS) --memory $(MINISHIFT_MEMORY) --disk-size $(MINISHIFT_DISK_SIZE) --vm-driver virtualbox --openshift-version="$(OPENSHIFT_VERSION)"
	./local-dev/minishift/minishift --profile $(MINISHIFT_PROFILE) openshift component add service-catalog
ifeq ($(ARCH), Darwin)
